...
// Retorna: { status, statusDetail, approved: boolean, raw }
// status pode ser: pending, approved, in_process, rejected, cancelled, refunded, charged_back
export async function getPaymentStatus(paymentId) {
  const token = process.env.MP_ACCESS_TOKEN;
  if (!token) {
    throw new Error("MP_ACCESS_TOKEN não está definido.");
  }
  if (!paymentId) {
    throw new Error("paymentId é obrigatório.");
  }

  const resp = await fetch(`${MP_API_URL}/${paymentId}`, {
  ...
  if (!resp.ok) {
    const msg =
      data?.message ||
      data?.error ||
      `Falha ${resp.status} ao consultar pagamento`;
    throw new Error(msg);
  }
